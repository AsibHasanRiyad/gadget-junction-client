import Marquee from "react-fast-marquee";

const AboutUs = () => {
  return (
    <div>
      <section className="bg-white dark:bg-gray-900">
        <div className="container px-6 py-10 mx-auto">
          <div className="lg:flex lg:items-center">
            <div className="w-full space-y-12 lg:w-1/2 ">
              <div>
                <h1 className="text-2xl font-semibold text-gray-800 capitalize lg:text-3xl dark:text-white">
                  Celebrating Innovation at Every <br /> Intersection
                </h1>

                <div className="mt-2">
                  <span className="inline-block w-40 mx-10 h-1 bg-blue-500 rounded-full"></span>
                  <span className="inline-block w-3 h-1 ml-1 bg-blue-500 rounded-full"></span>
                  <span className="inline-block w-1 h-1 ml-1 bg-blue-500 rounded-full"></span>
                </div>
              </div>

              <div className="md:flex md:items-start md:-mx-4">
                <span className="inline-block p-2 text-blue-500 bg-blue-100 rounded-xl md:mx-4 dark:text-white dark:bg-blue-500">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="w-6 h-6"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                    <path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" />
                  </svg>
                </span>

                <div className="mt-4 md:mx-4 md:mt-0">
                  <h1 className="text-xl font-semibold text-gray-700 capitalize dark:text-white">
                    A Hub of Cutting-Edge Technology:
                  </h1>

                  <p className="mt-3 text-gray-500 dark:text-gray-300">
                    Gadget Junction offers phones from renowned brands like
                    Apple, Samsung, Google, and many others. We stay up-to-date
                    with the latest releases to ensure that you have access to
                    the best in the market.
                  </p>
                </div>
              </div>

              <div className="md:flex md:items-start md:-mx-4">
                <span className="inline-block p-2 text-blue-500 bg-blue-100 rounded-xl md:mx-4 dark:text-white dark:bg-blue-500">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="w-6 h-6"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                    <path d="M12 6V4m0 2a2 2 0 100 4m0-4a2 2 0 110 4m-6 8a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4m6 6v10m6-2a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4" />
                  </svg>
                </span>

                <div className="mt-4 md:mx-4 md:mt-0">
                  <h1 className="text-xl font-semibold text-gray-700 capitalize dark:text-white">
                    Accessories
                  </h1>

                  <p className="mt-3 text-gray-500 dark:text-gray-300">
                    Enhance your phone with a range of accessories, including
                    protective cases, screen protectors, wireless chargers, and
                    more. Our accessory collection ensures that your device
                    remains functional and stylish.
                  </p>
                </div>
              </div>

              <div className="md:flex md:items-start md:-mx-4">
                <span className="inline-block p-2 text-blue-500 bg-blue-100 rounded-xl md:mx-4 dark:text-white dark:bg-blue-500">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="w-6 h-6"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                    <path d="M17.657 18.657A8 8 0 016.343 7.343S7 9 9 10c0-2 .5-5 2.986-7C14 5 16.09 5.777 17.656 7.343A7.975 7.975 0 0120 13a7.975 7.975 0 01-2.343 5.657z" />
                    <path d="M9.879 16.121A3 3 0 1012.015 11L11 14H9c0 .768.293 1.536.879 2.121z" />
                  </svg>
                </span>

                <div className="mt-4 md:mx-4 md:mt-0">
                  <h1 className="text-xl font-semibold text-gray-700 capitalize dark:text-white">
                    Performance Excellence
                  </h1>

                  <p className="mt-3 text-gray-500 dark:text-gray-300">
                    Our laptop collection includes powerful processors, ample
                    storage, high-resolution displays, and long-lasting battery
                    life. These features ensure that you can work efficiently
                    and enjoy multimedia content with ease.
                  </p>
                </div>
              </div>
            </div>

            <div className="hidden lg:flex lg:items-center lg:w-1/2 lg:justify-center">
              <img
                className="w-[28rem] h-[28rem] object-cover xl:w-[34rem] xl:h-[34rem] rounded-full"
                src="https://i.ibb.co/TWwgTgn/Screenshot-2023-10-18-at-3-59-58-PM.png"
                alt=""
              />
            </div>
          </div>

          <hr className="my-12 border-gray-200 dark:border-gray-700" />

          <div data-aos="fade-left" data-aos-offset="200" data-aos-easing="ease-in-sine" data-aos-duration="600" className=" flex text-[#676666] dark:text-white pt-2.5 text-sm font-medium">
      <Marquee pauseOnHover={false} className=" cursor-pointer">
      <img className=" w-40 mx-10" src="https://i.ibb.co/M2dZGj3/Png-Item-591019.png" alt="" />
      <img className=" w-40 mx-10" src="https://i.ibb.co/TKd21mH/samsung-logo-png-1304.png" alt="" />
      <img className=" w-40 mx-10" src="https://i.ibb.co/1bJPyFT/One-Plus-logo-PNG-jd6kdw.png" alt="" />
      <img className=" w-40 mx-10" src="https://i.ibb.co/wCgsbPP/google-logo-history-png-2593.png" alt="" />
      <img className=" w-40 mx-10" src="https://i.ibb.co/PDQBwdX/kisspng-logo-huawei-customer-service-centre-huawei-huawei-logo-modem-kurulumu-5b671020b645d8-4900495.png" alt="" />

   
      </Marquee>
    </div>
        </div>
      </section>
    </div>
  );
};

export default AboutUs;
